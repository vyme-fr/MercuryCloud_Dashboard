...
const route_name = "/services/order-form"
      ...
      if (result[0].token === req.query.token) {
        ...
        server.con.query(sql, function (err, result) {
          ...
          if (result[0].category == "pterodactyl") {           
            ...
            let data = {
              'name': result[0].name + " " + req.body.srv_info.srv_name + " (" + req.body.user_info.first_name + " " + req.body.user_info.last_name + ")",
              "user": 1,
              ...
              }).catch(err => {server.logger(" [ERROR] Pterodactyl API error : " + err); return req.json({"error": true, "code": 503, "msg": "Pterodactyl API error : " + err})}).then(() => {
