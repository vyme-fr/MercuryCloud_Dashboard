    ...
    if (json.error === false) {
      fetch(api_url + "users", {
        ...
        <tr>
            ...
            <td><a href="/services/service-game.html?id=${json.data[i].id}">${json.data[i].name}</a></td>
            ...
            <a class="btn btn-sm btn-icon btn-primary" data-toggle="tooltip" data-placement="top" title="" data-original-title="Modifier" href="/services/service-edit.html?id=${json.data[i].id}">
              <span class="btn-inner">
                ...
                <a class="btn btn-sm btn-icon btn-danger" onclick="delete_product('${json.data[i].id}')" data-toggle="tooltip" data-placement="top" title="" data-original-title="Supprimer">
                    <span class="btn-inner">
                        <svg width="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">                                
                  ...
                  console.error("[ERROR] Error in JSON3: " + JSON.stringify(json3))
                  //     window.location.replace("/errors/error500.html");
                }
              })
          } else {
            console.error("[ERROR] Error in JSON2: " + JSON.stringify(json2))
            //  window.location.replace("/errors/error500.html");
          }
        })
    } else {
      console.error("[ERROR] Error in JSON: " + JSON.stringify(json))
      // window.location.replace("/errors/error500.html");
    }
  })

function delete_product(id) {
  deleteData(api_url + "services/" + id).then(data => {
    if (data.error === false) {
      window.location.reload()
    } else {
      console.error('[ERROR] Code : ' + data.code + ' Message : ' + data.msg);
      if (data.code === 403) {
