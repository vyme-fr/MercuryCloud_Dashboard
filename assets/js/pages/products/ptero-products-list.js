...
fetch(`https://api.mercurycloud.fr/api/products/ptero-products?uuid=${getCookie("uuid")}&token=${getCookie("token")}`)
    ...
    for(var i= 0; i < myJson.products.length; i++) {
        ...
        <tr>
            ...
            <div class="flex align-items-center list-user-action">
                <a class="btn btn-sm btn-icon btn-success" data-toggle="tooltip" data-placement="top" title="" data-original-title="Créer" href="/dashboard/form/form-wizard.html?id=${myJson.products[i].id}">
                    ...
                    <a class="btn btn-sm btn-icon btn-warning" data-toggle="tooltip" data-placement="top" title="" data-original-title="Modifier" href="/dashboard/products/ptero-product-edit.html?id=${myJson.products[i].id}">
                    <span class="btn-inner">
                        <svg width="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">                                
                ...
                <a class="btn btn-sm btn-icon btn-danger" onclick="delete_product('${myJson.products[i].id}')" data-toggle="tooltip" data-placement="top" title="" data-original-title="Supprimer">
                    <span class="btn-inner">
                        <svg width="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">                                
...
function delete_product(id) {
    deleteData(`https://api.mercurycloud.fr/api/products/ptero-delete-product?uuid=${getCookie("uuid")}&token=${getCookie("token")}`, {"id": id}).then(data => {
        console.log(data)
        if (data.error == false) {
