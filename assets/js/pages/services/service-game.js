...
function load_files(directory) {
    document.getElementById("files-explorer").innerHTML = '<a class="list-group-item list-group-item-action">Chargement...</a>'
    function select_file(directory) { }
    fetch(`https://api.mercurycloud.fr/api/services/service-files?uuid=${getCookie("uuid")}&token=${getCookie("token")}&id=${url.searchParams.get('id')}&directory=${directory}`)
        ...
        .then(function (json_files) {
            if (json_files.error === false) {
                ...
                if (directory == "/") { document.getElementById("back-button").classList.add("disabled") } else { document.getElementById("back-button").classList.remove("disabled") }
                ...
                for (let i = 0; i < json_files.data.length; i++) {
                    if (json_files.data[i].attributes.name == "outof300fileslimit") {
                        ...
                        if (json_files.data[i].attributes.mimetype == "inode/directory") {
                            document.getElementById("files-explorer").innerHTML += `                              
                                        <div ondblclick="load_files('${directory + "/" + json_files.data[i].attributes.name}')" class="list-group-item list-group-item-action not-selectable">
                                            <svg width="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path opacity="0.4"
                                                    d="M16.8843 5.11485H13.9413C13.2081 5.11969 12.512 4.79355 12.0474 4.22751L11.0782 2.88762C10.6214 2.31661 9.9253 1.98894 9.19321 2.00028H7.11261C3.37819 2.00028 2.00001 4.19201 2.00001 7.91884V11.9474C1.99536 12.3904 21.9956 12.3898 21.9969 11.9474V10.7761C22.0147 7.04924 20.6721 5.11485 16.8843 5.11485Z"
                                                    fill="currentColor"></path>
                                         ...
                                         ${json_files.data[i].attributes.name}
                                         </div>`
                        } else {
                            document.getElementById("files-explorer").innerHTML += `                              
                                        <div ondblclick="console.log('fichier')" class="list-group-item list-group-item-action not-selectable">
            ...
            if (json.error === false) {
                ...
                socket.addEventListener('message', function (event) {
                    ...
                    if (data_parse.event == "stats") {
                        ...
                        document.getElementById("ram-counter").innerHTML = Math.round((args_parse.memory_bytes / (1024 * 1024 * 1024)) * 100) / 100 + "Go"
                        document.getElementById("disk-counter").innerHTML = Math.round((args_parse.disk_bytes / (1024 * 1024 * 1024)) * 100) / 100 + "Go"
                        ...
                        document.getElementById("ram-counter-span").innerHTML = "sur " + Math.round((args_parse.memory_limit_bytes / (1024 * 1024 * 1024)) * 100) / 100 + "Go"
