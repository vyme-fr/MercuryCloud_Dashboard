        ...
        for (let i = 0; i < json_files.data.length; i++) {
          if (json_files.data[i].attributes.name == "outof300fileslimit") {
            ...
            if (json_files.data[i].attributes.mimetype == "inode/directory") {
              document.getElementById("files-explorer").innerHTML += `<a id="${json_files.data[i].attributes.name}" onclick="select_file('${directory}', '${json_files.data[i].attributes.name}', ${i})" ondblclick="load_files('${directory + "/" + json_files.data[i].attributes.name}')" class="list-group-item list-group-item-action">
...
function select_file(directory, name, file_id) {
  if (ctrl_key_pressed == false) {
    ...
    <div class="form-group">
        <label class="form-label" for="name">Créé le : ${files_data[file_id].attributes.created_at}</label>
    </div>
    <div class="form-group">
        <label class="form-label" for="name">Modifé le : ${files_data[file_id].attributes.modified_at}</label>
    </div>
    <div class="form-group">
        <label class="form-label" for="name">Taille : ${calc_bytes(files_data[file_id].attributes.size)}</label>
    </div>
    <div class="form-group">
    ...
    <button onclick="save_file()" class="btn btn-primary col-md-12" id="save-file-btn">Enregistrer</button>
    <p></p>
  ...
  document.getElementById("delete-button").addEventListener("click", function () {
    ...
    postData(`https://dash.mercurycloud.fr:8000/api/services/${url.searchParams.get("id")}/files/delete?uuid=${getCookie("uuid")}&token=${getCookie("token")}`, body).then(data => {
      if (data.error == false) {
...
document.getElementById('file-upload').onchange = function () {
  ...
  postData(`https://dash.mercurycloud.fr:8000/api/services/${url.searchParams.get("id")}/files?uuid=${getCookie("uuid")}&token=${getCookie("token")}&directory=/`, {}).then(data => {
    if (data.error == false) {
      ...
      for (let i = 0; i < files.length; i++) {
        if ((files[i].size / 1024 / 1024) > 100) {
          ...
          document.getElementById("upload-button").innerHTML = `Max 100Mo !`;
      ...
      axios.post(data.data, formData, {
        ...
        document.getElementById("upload-button").classList.remove("disabled")
        document.getElementById("upload-button").innerHTML = `Télécharger`
        document.getElementById("progress-upload").setAttribute('aria-valuenow', 0)
        document.getElementById("progress-upload").setAttribute('style', 'width:' + 0 + '%')
        document.getElementById("progress-div").hidden = true
        load_files(back[back.length - 1]);
      ...
      if (json.error === false) {
        ...
        socket.addEventListener("message", function (event) {
          ...
          if (data_parse.event == "stats") {
            ...
            document.getElementById("net-counter").innerHTML = (Math.round(((args_parse.network.rx_bytes + args_parse.network.tx_bytes) / (1024 * 1024)) * 100) / 100) * 8 + "Mb/s";
            document.getElementById("cpu-counter-span").innerHTML = "sur "
            document.getElementById("disk-counter-span").innerHTML = "sur "
            document.getElementById("ram-counter-span").innerHTML = "sur " + calc_bytes(args_parse.memory_limit_bytes);
